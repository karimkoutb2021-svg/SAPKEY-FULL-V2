            ...
            <div className="md:hidden relative">
              ...
              {showProfileMenu && (
                <>
                  ...
                  <div className="absolute top-full left-0 mt-1 w-52 rounded-xl border bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shadow-lg z-50 py-1">
    ...
    <div className="flex flex-col h-full">
      ...
      {cart.length > 0 && (
        <div className={`border-t border-gray-100 dark:border-slate-800 space-y-2 shrink-0 ${isMobile ? 'p-2' : 'p-3'}`}>
          ...
          {/* Payment Methods */}
          <div className={`grid grid-cols-4 gap-1 ${isMobile ? 'gap-1' : 'gap-1.5'}`}>
            {[
            ...
            ].map(({ id, label, icon: I }: any) => (
              <button key={id}
                onClick={() => id === 'credit' ? onCreditClick?.() : setPaymentMethod(id)}
                className={`rounded-lg border text-center flex flex-col items-center justify-center transition-all ${isMobile ? 'h-9' : 'h-10'} ${
                  (id === 'credit' ? false : paymentMethod === id)
                    ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20'
                    : id === 'credit'
                    ? 'border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/30'
                    : 'border-gray-200 dark:border-slate-700 text-gray-500'
                }`}>
                <I className={`${isMobile ? 'h-3 w-3' : 'h-3.5 w-3.5'}`} style={(id === 'credit' ? {} : paymentMethod === id ? { color: primaryColor } : {})} />
                <span className={`${isMobile ? 'text-[7px]' : 'text-[8px]'} font-medium`}>{label}</span>
              </button>
            ))}
          </div>

...
export const customerWalletService = {
  async getWallet(userId: string): Promise<CustomerWallet | null> {
    let { data } = await supabase.from('customer_wallets').select('id, customer_id, balance, loyalty_points, total_recharged, total_spent, created_at').eq('user_id', userId).maybeSingle();
    if (!data) {
  ...
  async getTransactions(walletId: string): Promise<WalletTransaction[]> {
    const { data } = await supabase.from('wallet_transactions').select('id, wallet_id, customer_id, type, amount, description, reference, reference_id, status, created_at').eq('wallet_id', walletId).order('created_at', { ascending: false }).limit(50);
  ...
  async recharge(walletId: string, amount: number, method: 'cashier' | 'transfer_code' | 'card', reference?: string): Promise<CustomerWallet> {
    ...
    if (!data) {
      const { data: wallet } = await supabase.from('customer_wallets').select('id, customer_id, balance, loyalty_points, total_recharged, total_spent, created_at').eq('id', walletId).single();
  ...
  async redeemPoints(walletId: string, points: number): Promise<{ coupon: Coupon; wallet: CustomerWallet }> {
    ...
    const { data: updatedWallet } = await supabase.from('customer_wallets').select('id, customer_id, balance, loyalty_points, total_recharged, total_spent, created_at').eq('id', walletId).single();
    return { coupon: coupon as Coupon, wallet: updatedWallet as CustomerWallet };
  },

  async getCoupons(userId: string): Promise<Coupon[]> {
    const { data } = await supabase.from('coupons').select('id, code, discount_type, discount_value, min_purchase, is_used, expires_at').eq('user_id', userId).eq('is_used', false).order('created_at', { ascending: false });
...
export const customerAddressService = {
  async getAddresses(userId: string): Promise<any[]> {
    const { data } = await supabase.from('customer_addresses').select('id, user_id, label, address, city, is_default').eq('user_id', userId).order('is_default', { ascending: false });
    return (data || []);
  },
